...
void Render()
{
	...
	for (int i = MAX_ENTITIES - 1; i >= 0; i--)
	{
		if (g_entities[i].type != E_NULL)
		{
			...
			vec2 scaled_size = vmake(size.x * SPRITE_SCALE, size.y * SPRITE_SCALE);
			CORE_RenderCenteredSprite(offset_pos, scaled_size, g_entities[i].gfx);
		}
	}
...
void StartGame()
{
	...
	// Logic
	// Insert centered ship into entities
	vec2 pos = vmake(G_WIDTH / 2.0f, G_HEIGHT / 8.f);
	vec2 vel = vmake(0.f, VERTICAL_SHIP_VEL);
	InsertEntity(E_MAIN, pos, vel, MAINSHIP_RADIUS, g_ship_C);
}
...
void RunGame()
{
	...
	if (rand() < (RAND_MAX / 40)) 
	{
		vec2 pos = vmake((rand() * (float)G_WIDTH) / RAND_MAX, g_camera_offset + G_HEIGHT + 400.f);
		vec2 vel = vmake(0.f, 0.f);
		InsertEntity(E_ROCK, pos, vel, ROCK_RADIUS, g_rock[rand() % 5U]);
	}
