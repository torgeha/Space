...
//------------------------------------------------------------------------
inline float CORE_FRand(float from, float to) { return from + (to - from) * (rand() / (float)RAND_MAX); } // Return random float
inline unsigned CORE_URand(unsigned from, unsigned to) { return from + rand() % ((unsigned)(to - from + 1)); } // Return random unsigned
